...
@section('title', 'Edit Package Counter')
@section('content')
<h3 class="page-header">Edit Package Counter {{link_to_route('package_counters.index','Package Counter List',[],array('class'=>'btn btn-success pull-right'))}}</h3>

{{ Form::model($counter, ['route' => ['package_counters.update', $counter->id], 'method' => 'PUT', 'class' => 'form-horizontal']) }}
@csrf
<div class="row">
  <div class="col-md-6">
    ...
    <div class="form-group">
      ...
      <div style="max-height: 250px; overflow-y: auto; border: 1px solid #ddd; padding: 15px; border-radius: 4px; background: #f9f9f9;">
        @foreach($packages as $id => $name)
          <div class="checkbox" style="margin: 8px 0;">
            <label style="cursor: pointer; user-select: none;">
              <input type="checkbox" name="packages[]" value="{{ $id }}" {{ in_array($id, $selectedPackages) ? 'checked' : '' }}> {{ $name }}
            </label>
          </div>
        @endforeach
        @if(count($packages) === 0)
