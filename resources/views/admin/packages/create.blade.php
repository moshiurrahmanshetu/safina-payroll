...
@section('content')
<h3 class="page-header">Create Package</h3>
  ...
  <div class="col-md-8 col-md-offset-2">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        {{ Form::open(array('route' => 'packages.store', 'class' => 'form-horizontal')) }}

          <!-- Package Name -->
          <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
            {{ Form::label('name', 'Package Name *', array('class' => 'col-md-3 control-label')) }}
            <div class="col-md-9">
              {{ Form::text('name', old('name'), array('class' => 'form-control', 'required' => 'required', 'placeholder' => 'Enter package name')) }}
              @if ($errors->has('name'))
          ...
          <div class="form-group{{ $errors->has('base_price') ? ' has-error' : '' }}">
            ...
            <div class="col-md-9">
              <div class="input-group">
                ...
                {{ Form::number('base_price', old('base_price'), array('class' => 'form-control', 'required' => 'required', 'step' => '0.01', 'min' => '0', 'placeholder' => '0.00')) }}
              </div>
          ...
          <div class="form-group{{ $errors->has('default_person') ? ' has-error' : '' }}">
            ...
            <div class="col-md-9">
              {{ Form::number('default_person', old('default_person', 1), array('class' => 'form-control', 'required' => 'required', 'min' => '1', 'placeholder' => 'Number of persons included')) }}
          ...
          <div class="form-group{{ $errors->has('extra_person_price') ? ' has-error' : '' }}">
            ...
            <div class="col-md-9">
              <div class="input-group">
                <span class="input-group-addon">৳</span>
                {{ Form::number('extra_person_price', old('extra_person_price'), array('class' => 'form-control', 'required' => 'required', 'step' => '0.01', 'min' => '0', 'placeholder' => 'Price per extra person')) }}
              </div>
              ...
              <div class="checkbox-list" style="border: 1px solid #ddd; padding: 10px; border-radius: 4px; max-height: 200px; overflow-y: auto;">
                @foreach($tickets as $ticket_id => $ticket_name)
                  <div class="checkbox" style="margin-top: 5px; margin-bottom: 5px;">
                    <label>
                      <input type="checkbox" name="tickets[]" value="{{ $ticket_id }}" {{ is_array(old('tickets')) && in_array($ticket_id, old('tickets')) ? 'checked' : '' }}>
                      {{ $ticket_name }}
                    </label>
                  </div>
                @endforeach
          ...
          <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
            ...
            <div class="col-md-9">
              {{ Form::select('status', array('1' => 'Active', '0' => 'Inactive'), old('status', '1'), array('class' => 'form-control', 'required' => 'required')) }}
