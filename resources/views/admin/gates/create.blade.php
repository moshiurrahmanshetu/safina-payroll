...
@section('title', 'Ticket Counter Create')
@section('content')
<h3 class="page-header">Ticket Counter Create {{link_to_route('gates.index','Ticket Counter List',[],array('class'=>'btn btn-success pull-right'))}}</h3>
{{ Form::model(Request::old(),array('route' => array('gates.store'),'enctype'=>'multipart/form-data','class'=>'form-horizontal')) }}
<div class="row">
      ...
      <div class="form-group">
        ...
        <div style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; padding: 15px; border-radius: 4px; background: #f9f9f9;">
          @foreach($tickets as $id => $name)
            <div class="checkbox" style="margin: 8px 0;">
              <label style="cursor: pointer; user-select: none;">
                <input type="checkbox" name="tickets[]" value="{{ $id }}"> {{ $name }}
              </label>
            </div>
          @endforeach
          @if(count($tickets) === 0)
