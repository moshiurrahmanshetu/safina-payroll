...
@section('content')
<h3 class="page-header">Create Counter {{link_to_route('counters.index','Counter List',[],array('class'=>'btn btn-success pull-right'))}}</h3>

{{ Form::open(['route' => 'counters.store', 'class' => 'form-horizontal']) }}
<div class="row">
  <div class="col-md-6">
    ...
    <div class="form-group">
      ...
      <div style="max-height: 250px; overflow-y: auto; border: 1px solid #ddd; padding: 15px; border-radius: 4px; background: #f9f9f9;">
        @foreach($services as $service)
          <div class="checkbox" style="margin: 8px 0;">
            <label style="cursor: pointer; user-select: none;">
              <input type="checkbox" name="services[]" value="{{ $service->id }}"> {{ $service->service_category ? $service->service_category->name . ' - ' . $service->name : $service->name }}
            </label>
          </div>
        @endforeach
        @if(count($services) === 0)
...
<div class="row">
  <div class="col-md-12">
    <div class="form-group">
      ...
      {{ link_to_route('counters.index', 'Cancel', [], ['class' => 'btn btn-default']) }}
    </div>
  </div>
</div>
{{ Form::close() }}
