...
use App\Services\PackageCashHandoverCalculation;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class PackageCashHandoverController extends Controller
{
    protected $calculationService;

    public function __construct(PackageCashHandoverCalculation $calculationService)
    {
        parent::__construct();
    ...
    /**
     * Display cash handover dashboard for package bookings.
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.package_cash_handovers.index', compact('balanceData', 'businessDate'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $packageCounterId = $request->package_counter_id;

        // Validation: package_counter_id is required
        if (!$packageCounterId) {
        ...
        $existingPending = PackageCashHandover::where('user_id', $user->id)
        ...
        // Create handover request (store package_counter_id in counter_id field)
        PackageCashHandover::create([
            'user_id' => $user->id,
            'counter_id' => $packageCounterId,
        ...
        return redirect()->route('package_cash_handovers.index', ['business_date' => $businessDate])
            ->with('flash_success', 'Cash handover request created successfully');
    }

    /**
     * Display approval dashboard for package cash handovers.
     *
     * @return \Illuminate\Http\Response
     */
    public function approval(Request $request)
    {
        ...
        $query = PackageCashHandover::with(['user', 'counter']);
        ...
        $counters = \App\Models\PackageCounter::where('status', 1)->pluck('name', 'id');

        return view('admin.package_cash_handovers.approval', compact('handovers', 'users', 'counters'));
    }
    ...
    public function approve($id)
    {
        ...
        $handover = PackageCashHandover::findOrFail($id);
